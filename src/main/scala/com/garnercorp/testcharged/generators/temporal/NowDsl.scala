...
trait NowDsl {
  def now(
      ...
      fixedZone: ZoneOffset = ZoneOffset.UTC
  ...
  def now(fixedLocalDateTime: LocalDateTime): NowProvider = new NowProvider {
    override def apply(): Clock =
      Clock.fixed(ToInstant(fixedLocalDateTime), ZoneOffset.UTC)
  }

  def now(fixedZonedDateTime: ZonedDateTime): NowProvider = new NowProvider {
    override def apply(): Clock =
      Clock.fixed(ToInstant(fixedZonedDateTime), fixedZonedDateTime.getZone)
  }
}
