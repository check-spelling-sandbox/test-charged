...
object ZonedDateTimeGenerators
    extends TemporalGenerator[ZonedDateTime]
    with DateRanges {
  private type Edge = ZonedDateTime => ZonedDateTime
  private val Default: Edge = identity[ZonedDateTime]

  def apply(min: Edge = Default, max: Edge = Default)(implicit
      now: NowProvider
  ): Gen[ZonedDateTime] = for {
    createdInstant <- InstantGenerators(
      ToInstant(min(now.zonedDateTime)),
      ToInstant(max(now.zonedDateTime))
    )
