...
object FromInstant {
  ...
  def toLocalTime(instant: Instant): LocalTime =
    LocalDateTime.ofInstant(instant, ZoneOffset.UTC).toLocalTime
  def toLocalDateTime(instant: Instant): LocalDateTime =
    LocalDateTime.ofInstant(instant, ZoneOffset.UTC)
  def toZonedDateTime(instant: Instant, zone: ZoneId): ZonedDateTime =
    instant.atZone(zone)
}
