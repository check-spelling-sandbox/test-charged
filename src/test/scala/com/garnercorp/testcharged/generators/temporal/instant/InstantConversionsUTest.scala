...
class InstantConversionsUTest extends AnyFunSuite with Matchers {
  test("Can convert from a LocalDate to an Instant and back to a LocalDate") {
    ...
    val testLocalDateFromInstant =
      FromInstant.toLocalDate(testLocalDateAsInstant)
  ...
  test("Can convert from a LocalTime to an Instant and back to a LocalTime") {
    ...
    val testLocalTimeFromInstant =
      FromInstant.toLocalTime(testLocalTimeAsInstant)

    testLocalTime shouldBe testLocalTimeFromInstant
  }

  test(
    "Can convert from a LocalDateTime to an Instant and back to a LocalDateTime"
  ) {
    val testLocalDate = LocalDate.of(2018, Month.JUNE, 20)
    val testLocalTime = LocalTime.of(4, 30)
    ...
    val testLocalDateTimeFromInstant =
      FromInstant.toLocalDateTime(testLocalDateTimeAsInstant)

    testLocalDateTime shouldBe testLocalDateTimeFromInstant
  }

  test(
    "Can convert from a ZonedDateTime to an Instant and back to a ZonedDateTime"
  ) {
    val testLocalDate = LocalDate.of(2018, Month.JUNE, 20)
    val testLocalTime = LocalTime.of(4, 30)
    val testLocalDateTime = LocalDateTime.of(testLocalDate, testLocalTime)
    ...
    val testZonedDateTimeFromInstant =
      FromInstant.toZonedDateTime(testZonedDateTimeAsInstant, testZone)
